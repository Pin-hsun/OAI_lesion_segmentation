...
if __name__ == "__main__":
    ...
    from loaders.dataset_brain import DataBrain
    train_set = DataBrain(source='/media/ghc/GHc_data1/Dataset/heddata/brain0708/patches/train/')
    eval_set = DataBrain(source='/media/ghc/GHc_data1/Dataset/heddata/brain0708/patches/test/')

    train_loader = DataLoader(train_set, batch_size=args['batch_size'], shuffle=True, num_workers=16, drop_last=False)
    eval_loader = DataLoader(eval_set, batch_size=args['batch_size'], shuffle=False, num_workers=16, drop_last=False)
    """ Imports """
    from models.unet import UNet_clean
    net = UNet_clean(output_ch=2, backbone=args['backbone'], depth=args['depth'])
