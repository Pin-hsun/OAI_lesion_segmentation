...
class LitClassification(nn.Module):#pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx=0):
        ...
        imgs, labels, _ = batch
        labels = (labels == 1).type(torch.float32)
    ...
    def validation_step(self, batch, batch_idx=0):
        ...
        # metrics
        self.all_label.append(labels0.cpu())
        #self.all_out.append(nn.Softmax(dim=1)(output[0]).cpu().detach())
        self.all_out.append(output[0][:, 1:, ::].cpu().detach())
    ...
    def overall_loop(self):
        ...
        for epoch in range(self.args['epochs']):
            ...
            if (epoch % 5) == 0:
                torch.save(self.net, 'checkpoints/' + str(epoch) + '.pth')