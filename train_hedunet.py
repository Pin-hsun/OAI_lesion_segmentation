...
from engine.lightning_classification_hedunet import LitClassification
import torch
...
def sanity_check(train_set, net, loss_function):
    img, target, _ = train_set.__getitem__(100)
    target = torch.from_numpy(target == 1).type(torch.float32)
    (img, target) = (x.unsqueeze(0) for x in (img, target))

    # forward
    y_hat, y_hat_levels = net(img.cuda())
...
if __name__ == "__main__":
    ...
    args['dir_checkpoint'] = os.getenv("HOME") + '/Dataset/OAI_DESS_segmentation/checkpoints/'

    args_d = {'mask_name': 'bone_resize_B_crop_00',
              'data_path': os.getenv("HOME") + '/Dataset/OAI_DESS_segmentation/',
              'mask_used': [['femur', 'tibia']],
              'scale': 0.5,
              'interval': 1,
              'thickness': 0,
              'method': 'automatic'}
    ...
    """ Imports """
    from models.hedunet.hed_unet import HEDUNet
    net = HEDUNet(input_channels=3, output_channels=len(args_d['mask_used']) + 1)
    from utils.metrics_hedunet import HedUnetLoss
    loss_function = HedUnetLoss()
    from utils.metrics_segmentation import SegmentationDiceCoefficient, SegmentationCrossEntropyLoss
...
# CUDA_VISIBLE_DEVICES=0 python train_hedunet.py -b 16 --bu 64 --lr 0.01
